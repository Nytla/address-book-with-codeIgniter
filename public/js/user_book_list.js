	...
	$.ajaxes = function(img_details_open) {
		...
		$.ajax({  
			...
			success: function(object) {
				...
				var image_path = '/public/images/uploads_client/';
				
				var no_notes = 'No information available.';
				...
				$(object).each(function(i, v) {
					...
					content += '<img src="' + image_path + v.photo_name + '" height="' + v.photo_height + '" width="' + v.photo_width + '" alt="' + v.photo_description + '" class="left" style="padding: 10px;">';

					content += '<p style="padding: 10px;">';

					content += '<b>Email:</b> ' + v.email + '<br>';

					notes = (v.notes) ? v.notes : no_notes;
					
					content += '<b>Notes:</b> ' + notes + '';
